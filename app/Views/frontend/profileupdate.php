                     ...
                     <div class="card card-with-shadow-sm mb-3">
                        <div class="card-body">
                           ...
                           <div class="d-flex align-items-center justify-content-between mb-3">
                            ...
                            <input type="text" class="form-control <?php if (session('errors.firstname')) : ?>is-invalid<?php endif ?>"
                                   name="firstname" aria-describedby="firstnameHelp" placeholder="First Name" value="<?= old('firstname') ? old('firstname') : user()->firstname ?>" required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control <?php if (session('errors.lastname')) : ?>is-invalid<?php endif ?>"
                                   name="lastname" aria-describedby="lastnameHelp" placeholder="Last Name" value="<?= old('lastname') ? old('lastname') : user()->lastname ?>" required>
                        ...
                        <div class="form-group">
                            ...
                            <select name="bgroup"  class="form-control <?php if (session('errors.bgroup')) : ?>is-invalid<?php endif ?>" required>
                                       ...
                                       <option  value="A+" <?php if (old('bgroup') === 'A+' || user()->bgroup === 'A+') { print('selected'); } ?>>A+</option>
                                       <option  value="B+" <?php if (old('bgroup') === 'B+' || user()->bgroup === 'B+') { print('selected'); } ?>>B+</option>
                                       <option  value="AB+" <?php if (old('bgroup') === 'AB+' || user()->bgroup === 'AB+') { print('selected'); } ?>>AB+</option>
                                       <option  value="O+" <?php if (old('bgroup') === 'O+' || user()->bgroup === 'O+') { print('selected'); } ?>>O+</option>
                                       <option  value="O-" <?php if (old('bgroup') === 'O-' || user()->bgroup === 'O-') { print('selected'); } ?>>O-</option>
                                       <option  value="A-" <?php if (old('bgroup') === 'A-' || user()->bgroup === 'A-') { print('selected'); } ?>>A-</option>
                                       <option  value="B-"<?php if (old('bgroup') === 'B-' || user()->bgroup === 'B-') { print('selected'); } ?>>B-</option>
                                       <option  value="AB-" <?php if (old('bgroup') === 'AB-' || user()->bgroup === 'AB-') { print('selected'); } ?>>AB-</option>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control <?php if (session('errors.phonenumber')) : ?>is-invalid<?php endif ?>" name="phonenumber" placeholder="Phone Number" value="<?= old('phonenumber') ? old('phonenumber') : user()->phonenumber ?>" required>
                                <small id="emailHelp" class="form-text text-muted">We'll never share your phone number with anyone else.</small>
                        </div>

                         <div class="form-group">
                            <label for="haddress">Home Address</label>
                            <textarea type="text" name="haddress" class="form-control <?php if (session('errors.haddress')) : ?>is-invalid<?php endif ?>"  placeholder="Home Address" required><?= old('haddress') ? old('haddress') : user()->haddress ?></textarea>
                        </div>

                        <div class="form-group">
                            <label for="institute">Educational Institute</label>
                            <input type="text" class="form-control <?php if (session('errors.institute')) : ?>is-invalid<?php endif ?>" name="institute" placeholder="Educational Intitute" value="<?= old('institute') ? old('institute') : user()->institute ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="batch">Batch</label>
                            <input type="text" class="form-control <?php if (session('errors.batch')) : ?>is-invalid<?php endif ?>" name="batch" placeholder="Batch-21" value="<?= old('batch') ? old('batch') : user()->batch ?>" required>
                        </div>
